...
use App\Libraries\RiskMatrixAiEngine;
    ...
    public function edit()
    {
        ...
        if ($rows === null && $missionId) {
            ...
            if (empty($rows)) {
                ...
                // نقترح صفوفًا مبدئية بمجرد اختيار المهمة (تحليل محلي قائم على
                // كلمات مفتاحية، بدون أي استدعاء لخدمة ذكاء اصطناعي خارجية،
                // يجمع نصه من كل بيانات المهمة الفعلية: بيانات المهمة، اتفاقية
                // مستوى الخدمة، مذكرة تخطيط المهمة، قائمة المستندات المطلوبة،
                // ونص الملفات المرفوعة فعليًا -- بدل جدول فارغ يحتاج تعبئة يدوية كاملة
                $rows = (new RiskMatrixAiEngine())->suggest($mission);
                $aiSuggested = true;
